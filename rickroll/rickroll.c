...
static char *rickroll_filename = "/home/luisbg/music/rickroll.mp3";
...
asmlinkage long rickroll_open(const char __user *filename, int flags,
			      umode_t mode);
...
static int __init rickroll_init(void)
{
	...
	/*
	 * Replace the entry for open with our own function. We save the
	 * location of the real sys_open so we can put it back when we're
	 * unloaded.
	 */
